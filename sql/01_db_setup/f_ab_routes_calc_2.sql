-- FUNCTION: pgr.ab_routes_calc_2(text, text, integer)

-- DROP FUNCTION pgr.ab_routes_calc_2(text, text, integer);

CREATE OR REPLACE FUNCTION pgr.ab_routes_calc_2(
	_val1 text,
	_val2 text,
	_buffer integer)
    RETURNS text
...
EXECUTE 'INSERT INTO pgr.ab_routes
		 ...
		 FROM (SELECT anodestopid, bnodestopid, anodexcoord, anodeycoord, bnodexcoord, bnodeycoord,
			  (SELECT geom_ FROM pgr.get_shortest_path_2(anodestopid, bnodestopid,' || ($3) ||')),
		 ...
		 SET geom = (SELECT geom_ FROM pgr.get_shortest_path_2(astopid, bstopid,' || 20*($3) ||'))
			,buffer_m = ' || 20*($3) ||'
...
ALTER FUNCTION pgr.ab_routes_calc_2(text, text, integer)
    OWNER TO postgres;
