-- FUNCTION: pgr.get_shortest_path_4b(character varying, character varying, integer)

-- DROP FUNCTION pgr.get_shortest_path_4b(character varying, character varying, integer);

CREATE OR REPLACE FUNCTION pgr.get_shortest_path_4b(
...
q2 text := E'' || 'circ_edge AS (SELECT geom FROM pgr.openroads WHERE gid = ' || b_edge_id || '),';
q3 text := E'' || 'circ_frac_1 AS (SELECT ST_LineLocatePoint((SELECT geom FROM circ_edge),
																(SELECT pgr.get_node_geom(''' || $2 || ''')))),';
q4 text := E'' || 'circ_frac_2 AS (SELECT ST_LineLocatePoint((SELECT geom FROM circ_edge),
																(SELECT the_geom FROM pgr.openroads_vertices_pgr 
																WHERE id = ' || b_node_id || '))),';
q5 text := E'' || 'circ_end AS (SELECT ST_LineSubstring(
...
q6 text := E'' || 'SELECT ST_LineMerge(ST_Union(ARRAY[(SELECT ST_Union(geom) FROM route),(SELECT * FROM circ_end)]))';
...
ALTER FUNCTION pgr.get_shortest_path_4b(character varying, character varying, integer)
    OWNER TO postgres;
