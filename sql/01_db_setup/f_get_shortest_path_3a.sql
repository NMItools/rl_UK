-- FUNCTION: pgr.get_shortest_path_3a(character varying, character varying, integer)

-- DROP FUNCTION pgr.get_shortest_path_3a(character varying, character varying, integer);

CREATE OR REPLACE FUNCTION pgr.get_shortest_path_3a(
...
a_edge_id int := (SELECT pgr.get_nearest_edge_2($1, a_node_id));

q1 text :='WITH route AS (SELECT * FROM pgr.openroads JOIN(SELECT * FROM pgr_dijkstra(''SELECT gid as id, source, target, st_length(geom) as cost FROM pgr.openroads WHERE geom && ST_Expand(ST_Envelope(ST_Collect(ST_GeomFromText(''''' || geom_a || '''''), ST_GeomFromText(''''' || geom_b ||'''''))),'||$3||')''' ||', ' || a_node_id || ', ' || b_node_id || ',false)) AS route ON pgr.openroads.gid = route.edge),';
q2 text := E'' || 'end_route AS (SELECT CASE WHEN (SELECT source FROM route WHERE path_seq = 1) = ' || a_node_id || '
					   ...
					   -- 2
								   ELSE ( SELECT CASE WHEN (SELECT edge FROM route WHERE path_seq = 1)=' || a_edge_id || '
													  ...
													  END)
					  END),';
q3 text := E'' || 'start_edge AS (SELECT geom FROM pgr.openroads WHERE gid = ' || a_edge_id || '),';
q4 text := E'' || 'start_frac_1 AS (SELECT ST_LineLocatePoint((SELECT geom FROM start_edge),(SELECT pgr.get_node_geom(''' || $1 || ''')))),';
...
ALTER FUNCTION pgr.get_shortest_path_3a(character varying, character varying, integer)
    OWNER TO postgres;
